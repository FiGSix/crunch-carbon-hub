...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `[Cession Email] Sending confirmation email for proposal: ${proposalId} to ${clientEmail}`,
    );
    ...
    console.log(
      `[Cession Email] Fetching signed agreement PDF for proposal: ${proposalId}`,
    );
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      const { data, error } = await supabase
        ...
        .select(
          "pdf_path, signed_pdf_url, typed_name, client_cession_signature_id, metadata",
        )
        .eq("proposal_id", proposalId)
      ...
      if (error) {
        console.error(
          `[Cession Email] Attempt ${attempt} - error fetching agreement:`,
          error,
        );
      }
      agreement = data ?? null;
      ...
      if (attempt < 3) {
        console.log(
          `[Cession Email] Signed document not present on attempt ${attempt}, waiting 1500ms...`,
        );
    ...
    if (!signatoryName && agreement?.client_cession_signature_id) {
      ...
      signatoryName =
        (
          masterSignature?.metadata?.["signatory_name"] as string | undefined
        )?.trim() ||
    ...
    if (!signedRef) {
      console.warn(
        "[Cession Email] ⚠️ Signed PDF still not available after retries — attaching UNSIGNED proposal PDF as fallback. Investigate generate-signed-agreement-pdf logs for this proposal.",
      );
    } else {
      console.log(
        "[Cession Email] Using signed agreement document:",
        signedRef,
      );
    }
    ...
    if (pdfUrl) {
      try {
        ...
        const asPath = (value: string, bucket: string): string | null => {
          const m = value.match(
            new RegExp(
              `/object/(?:public|sign|authenticated)/${bucket}/([^?]+)`,
            ),
          );
        ...
        const signedPath = isSignedDoc
          ...
          : url.includes("/signed-agreements/")
            ? asPath(url, "signed-agreements")
            : null;
        ...
        if (signedPath) {
          console.log(
            `[Cession Email] Downloading from signed-agreements: ${signedPath}`,
          );
          ...
          console.log(
            `[Cession Email] Downloading from proposal-pdfs: ${proposalPath}`,
          );
          const { data: blob, error } = await supabase.storage
            .from("proposal-pdfs")
            .download(proposalPath);
          if (error) throw error;
          pdfBuffer = await blob!.arrayBuffer();
        } else {
        ...
        if (pdfBuffer) {
          ...
          console.log(
            `[Cession Email] PDF fetched, size: ${pdfBuffer.byteLength} bytes`,
          );
        } else {
    ...
    if (pdfAttachment) {
      ...
      console.log(
        "[Cession Email] Sending email without attachment (PDF fetch failed)",
      );
    }
    ...
    console.error(
      "[Cession Email] Error in send-cession-agreement-email function:",
      error,
    );
      ...
      JSON.stringify({
        error:
          error instanceof Error
            ? error.message
            : "Failed to send cession agreement email",
