...
import {
  authorizeCompanySigner,
  resolveStoredSignatoryName,
} from "./signer-authorization.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Accepting proposal with ${token ? `token: ${token.substring(0, 8)}...` : `proposalId: ${proposalId}`}`,
    );
    ...
    if (proposal.status === "rejected") {
      ...
      return new Response(
        JSON.stringify({
          error: "This proposal has been rejected and cannot be signed",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (
      ...
      new Date(proposal.invitation_expires_at) < new Date()
    ) {
      return new Response(
    ...
    if (
      signatureType === "typed_name" &&
      (!typedName || typedName.trim().length < 2)
    ) {
      console.error("❌ Invalid typed name for typed signature:", {
        typedName,
      ...
      return new Response(
        JSON.stringify({
          error:
            "Please provide a valid name (minimum 2 characters) when using typed signature",
    ...
    if (bearerToken && bearerRole !== "anon") {
      ...
      authenticatedProfileName =
        [profile?.first_name, profile?.last_name]
          ...
          .join(" ")
          .trim() || null;
      authenticatedProfileEmail =
        profile?.email?.trim() || authData.user.email || null;
    }
    ...
    if (companyCedent && resolvedSignatory.length < 2) {
      return new Response(
        JSON.stringify({
          error:
            "Please provide the full name of the person signing on behalf of the company",
    ...
    console.log(
      `✅ Signature validation passed for ${signatureType} signature`,
    );
    ...
    if (isReferral) {
      ...
      if (!pd.systemAddress || pd.systemAddress.trim().length < 5)
        missing.push("systemAddress");
      if (!pd.commissioningDate) missing.push("commissioningDate");
      if (!pd.installerCompanyName || pd.installerCompanyName.trim().length < 2)
        missing.push("installerCompanyName");
      if (
        !pd.installerEmail ||
        !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(pd.installerEmail)
      )
        missing.push("installerEmail");
    ...
    const dbSignatureType =
      signatureType === "canvas" ? "electronic_signature" : "typed_name";
    ...
    if (!liveDoc?.id) {
      console.error(
        "❌ No live cession agreement configured — refusing to record a signature",
      );
      return new Response(
        JSON.stringify({
          error:
            "No live Cession Agreement is configured. Please contact support.",
    ...
    let masterDocVersion: number = liveDoc.current_version;
    {
      ...
      if (existingMaster) {
        ...
        masterDocVersion =
          existingMaster.legal_document_version ?? liveDoc.current_version;
        console.log(
          `ℹ️ Reusing existing master signature ${masterSignatureId}`,
        );
      } else {
        ...
        if (masterError || !createdMaster) {
          console.error(
            "❌ Failed to create master cession signature:",
            masterError,
          );
        ...
        console.log(
          `✅ Master cession signature created: ${masterSignatureId}`,
        );
      }
    }
    ...
    console.log(
      `✅ Proposal ${proposal.id} successfully signed via ${signatureType}`,
    );
    ...
    if (isReferral && projectDetails && onboardingProjectId) {
      try {
        ...
        const installerCompanyName =
          projectDetails.installerCompanyName?.trim();
        const installerEmail = projectDetails.installerEmail
          ?.trim()
          .toLowerCase();
        if (
          !systemAddress ||
        ...
        const { error: invErr } = await supabase.functions.invoke(
          "send-installer-invitation",
          {
            body: { proposalId: proposal.id },
          },
        );
        if (invErr)
          console.error("[accept-proposal] installer invite failed", invErr);
      } catch (e) {
        console.error(
          "[accept-proposal] project details / installer invite error",
          e,
        );
      }
    }
    ...
    const postSign = (async () => {
      try {
        ...
        const { data: signedPdfResult, error: pdfError } =
          await supabase.functions.invoke("generate-signed-agreement-pdf", {
            body: { proposalId: proposal.id, agreementId: newAgreement.id },
          });

        if (pdfError) {
          console.error(
            "❌ [post-sign] Failed to generate signed PDF:",
            pdfError,
          );
        } else {
          console.log(
            "✅ [post-sign] Signed agreement PDF generated:",
            signedPdfResult?.signed_pdf_url,
          );
        }

        if (clientEmail) {
          console.log(
            `📧 [post-sign] Sending cession agreement email to ${clientEmail}`,
          );
          ...
          if (emailError) {
            console.error(
              "❌ [post-sign] Cession email send failed:",
              emailError,
            );
          } else {
          ...
          console.warn(
            "⚠️ [post-sign] No client email found, skipping confirmation email",
          );
        }
        ...
        const { data: sweepResult, error: sweepError } =
          await supabase.functions.invoke("sweep-agreement-documents", {
            body: { clientId: masterClientId },
          });
        if (sweepError) {
          console.error(
            "❌ [post-sign] Sibling document sweep failed:",
            sweepError,
          );
        } else if (sweepResult?.processed) {
          console.log(
            `✅ [post-sign] Generated ${sweepResult.processed} inherited sibling document(s)`,
          );
        }
      } catch (err) {
